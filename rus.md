# Разрабатываем хорошие API на JavaScript
...
Обратите внимание, нам не пришлось хранить ссылку на элемент в переменной и
...
надо ли использовать точки с запятой. В статье
«[Расширение встроенных нативных объектов. Зло или нет?][11]» [kangax][12]
рассказывет о плюсах и минусах такого подхода. Хотя у каждого есть своё
мнение, все сходятся в одном: в таких вещах нужно единообразие. Впрочем, даже
...
ваш эквивалент `.foo()` не является общим термином и что маловероятно
пересечение с другими API. Убедитесь, что вы должным образом предоставили
...
Это определение гласило просто: [*не PHP*][16]. Не. Дай. Бог. Вы вздумаете
назвать функцию вроде *str_repeat()*, *strpos()*, *substr()*. Также никогда
в жизни не меняйте позиции аргументов. Если вы объявили в каком-то одном месте
...
`css()`, он может принимать словарь событий, но он этим не ограничивается и
позволяет зарегистрировать один обработчик на несколько событий:
...
В JavaScript это пока невозможно. Хотя в «следующую версию JavaScript» (её
часто называют ES.next, ES6 или Harmony) войдут
...
любом другом современном API) решено использовать концепцию «объект с
опциями».
...
в будущем мы захотим добавить новую фичу.
...
ощущаться как раздутое и непонятное. Обращайте внимание только на те случаи,
...
виджетами UI. Библиотеки вроде jQuery предоставляют простые интерфейсы, и с
их помощью вы можете легко овладеть этой областью.

Интерфейс событий, как подсказывает название, лучшего всего применим там,
где что-то происходит. То, нужно ли отображать какой-нибудь виджет,
...
Как видите, куча повторяющегося кода. Принцип «не повторяйся» (DRY)
подсказывает воспользоваться шаблоном-генератором:
...
неудачи, или независимо от состояния.
См. «[Конвееры промисов в JavaScript][45]» для более детального описания
дефёрредов.
...
Как и с любым кодом, *разработка через тестирование* (TDD) — это простой способ
...
она не заменяет её полностью.

Кто-то в интернете советует выставлять каждый копонент цепочки вызовов в своей
...
SVN и GIT) тоже могут от этого выиграть. А что касается отладки, только в
...
Вы должны писать самоочевидный код. Чаще всего это не проблема,
...
тратите время, как своё, так и читателя. Используйте комментарии, объясняющие,
*почему* вы решили проблему именно так, а не *как* вы решили проблему.
«Как» должно быть очевидно из вашего кода, так что не повторяйтесь.
Обратите внимание, что использование комментариев для визуальной разметки
блоков вашего кода или для объяснения общих принципов вполне допустимо.
...
*   Гибкие API хороши, а те API, что не дадут допустить ошибку, ещё лучше.
